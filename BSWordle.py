...
class BlueskyWordle:
    ...
    def dataframe_to_list(self, df):
        ...
        if self.filter_posts:
            self.df = self.df.filter(pl.col("post_text").str.contains(WORDLE_SHARE))
        self.first_guesses = self.df["first_guess"].explode().drop_nulls().to_list()
        print(
            f"Bulding pattern list for {self.wordle_num} using {self.df.shape[0]} posts."
        )
            ...
            for word, d in self.zipped_counters.items():
                if max_count >= guess_count >= min_count:
                    ...
                    if res is not None:
                        ...
                        res_dict[word]["impossible_pattern_count"] += max(
                            1, guess_count - 1
                        )
    ...
    def get_solution_from_db(self, wordle_num):
        with sqlite3.connect("wordle.db") as con:
            res = con.execute(
                "select solution from hashed_solutions where wordle_id = ? ",
                (wordle_num,),
            ).fetchone()
        if res is not None:
            return res[0]
    ...
    def load_solution(self, wordle_num):
        ...
        if self.hashed_solution not in self.hashed_keys:
            print(
                "** Impossible to solve with loaded dictionary. Solution data missing. **"
            )
            return False
        return True
    ...
    def solve_df(
        ...
        if filter_pattern:
            df = df.filter(
                pl.col("post_text")
                .str.to_lowercase()
                .str.contains(f"wordle {wordle_num:,}")
            )
        ...
        for p in range(-7, -300, -5):
            ...
            res = (
                ...
                .with_columns(
                    norm_score=(
                        pl.col("adjusted_score") - pl.col("adjusted_score").mean()
                    )
        ...
        best_df = best_df.with_columns(
            fraction_found=pl.col("valid_patterns_found")
            / pl.col("total_valid_patterns")
        ).with_columns(
            kstatistic=pl.col("word").map_elements(
                self.ks_stat, return_dtype=pl.Float64
            ),
